...
public:
    ...
    SymbolTable(const TokenList &tokens) : tokens_(tokens) {}
...
private:
	...
	TokenSequence Cin;
	// vector<Token> Token_list = Cin.getToken_list();
    TokenList tokens_;
	int funcnum = 0;				//符号表中元素个数
};

//判断函数是否已经存在 并返回在该函数在函数名表中的位置 不存在则返回-1
int SymbolTable::func_exist(struct func functable[255], int funcnum, string str)
{
	int i;
	for (i = 0; i < funcnum; i++)
...
//判断变量是否出现在当前函数的符号表中 存在则返回表中位置 否则返回-1
int SymbolTable::sym_exist(struct func functable[255], struct symbol* st, int num, string str)
{
...
// 获得当前变量的类型
string SymbolTable::getType(string var, int num) {
	int i = 0;
...
// 获得当前变量的属性
string SymbolTable::getProperty(string var, int num) {
	int i = 0;
...
//将当前函数名加入到当前函数表中
void SymbolTable::pushsym(struct func functable[255], struct symbol* st, int curnum, int num) // curnum:当前函数编号
{
...
//将当前函数加入到函数表中 并返回该函数符号表的地址
struct symbol* SymbolTable::pushfunc(struct func functable[255], int& funcnum, string str, string ret)
{
...
//初始化符号表
void SymbolTable::init_symtable(void)
{
...
void SymbolTable::output()
{
	...
	int set = 20;
	//输出函数符号表
	cout << setw(set) << left << "函数名" << setw(set) << "函数中参数个数" << setw(set) << "函数返回类型" << setw(set) << "函数符号表地址" << endl;
	for (i = 0; i < funcnum; i++)
		cout << setw(set - 1) << left << functable[i].name << setw(set - 2) << functable[i].num << setw(set - 2) << functable[i].ret << "0x" << setw(set) << functable[i].st << endl;
	//输处各函数表
	for (i = 0; i < funcnum; i++)
	{
		cout << endl;
		cout << "函数名:" << functable[i].name << "\t符号表地址:0x" << functable[i].st << endl;
		if (functable[i].num)
			cout << setw(set) << "变量名" << setw(set) << "属性" << setw(set) << "类型" << setw(set) << "地址" << endl;
		for (j = 0; j < functable[i].num; j++)
			cout << setw(set - 1) << functable[i].st[j].name << setw(set - 1) << functable[i].st[j].property << setw(set - 1) << functable[i].st[j].type << setw(set) << functable[i].st[j].st << endl;
	}
}
...
void SymbolTable::calsymboltable()
{
	...
	int flag1, flag2, num, curnum = 0;
	for (auto iter = tokens_.begin(); iter != tokens_.end(); iter++)
	{
		if ((iter->value == "+") || ((*iter).value == "-") || ((*iter).value == "*") || ((*iter).value == "'\'"))
		{
			iter->type = "OOP";
		}
		else if ((*iter).type == "I")
